...
Dashboard.implement({
  ...
  incrementRunningCalls: function() {
    ...
    this.log("+Running calls incremented to: " + this.getRunningCalls());
  },
  ...
  decrementRunningCalls: function() {
    this.runningCalls--;
    this.log("-Running calls decremented to: " + this.getRunningCalls());
    setTimeout(_.bind(function(){
  ...
  init: function(components){
    ...
    this.log("InitInstance " + initInstance);
  ...
  _handlePostInit: function(initInstance) {
    ...
    if( (!myself.waitingForInit || myself.waitingForInit.length === 0) && !myself.finishedInit ) {
      ...
      if( myself.logLifecycle && typeof console != "undefined" ){
        console.log("%c          [Lifecycle <End  ] Init[" + initInstance + "] (Running: "+ myself.getRunningCalls()  +")","color: #ddd ");
      }
