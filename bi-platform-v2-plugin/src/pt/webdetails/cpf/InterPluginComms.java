...
public class InterPluginComms
{
  ...
  public static boolean isPluginAvailable(String plugin) {
    ...
    try
    {
      contentGenerator = pluginManager.getContentGeneratorForType(plugin, userSession);
    }
    catch (Exception e)
    {
      return false;
  ...
  public static String callPlugin(Plugin plugin, String method, IParameterProvider params, boolean switchClassLoader)
  {
    ...
    try
    {
      contentGenerator = pluginManager.getContentGeneratorForType(plugin.getName(), userSession);
    }
    catch (Exception e)
    {
