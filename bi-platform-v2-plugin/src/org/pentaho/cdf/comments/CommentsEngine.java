...
public class CommentsEngine {
  ...
  public String process( IParameterProvider requestParams, IPentahoSession userSession )
    ...
    try {
      ...
      logger.error( Messages.getErrorString( "DashboardDesignerContentGenerator.ERROR_001_INVALID_METHOD_EXCEPTION" )
          + " : " + actionParam );
      throw new InvalidCdfOperationException( ex );
    }
  ...
  public JSONObject add( IParameterProvider requestParams, IPentahoSession userSession ) throws JSONException,
    InvalidCdfOperationException, PluginHibernateException {
  ...
  public JSONObject list( IParameterProvider requestParams, IPentahoSession userSession ) throws JSONException,
    InvalidCdfOperationException, PluginHibernateException {
  ...
  public JSONObject delete( IParameterProvider requestParams, IPentahoSession userSession ) throws JSONException,
    InvalidCdfOperationException, PluginHibernateException {
  ...
  public JSONObject archive( IParameterProvider requestParams, IPentahoSession userSession ) throws JSONException,
    InvalidCdfOperationException, PluginHibernateException {
    int commentId = Integer.parseInt( requestParams.getStringParameter( "commentId", "" ) );
  ...
  private JSONObject changeCommentStatus( int operationType, int commentId, IParameterProvider requestParams,
      IPentahoSession userSession ) throws JSONException, PluginHibernateException {
    Session session = getSession();
    session.beginTransaction();
    CommentEntry comment = (CommentEntry) session.load( CommentEntry.class, commentId );
    switch ( operationType ) {
      case DELETE_OPERATION:
        if ( operationAuthorized( operationType, comment, userSession ) )
          comment.setDeleted( Boolean.valueOf( requestParams.getStringParameter( "value", "true" ) ) );
        break;
      case ARCHIVE_OPERATION:
        if ( operationAuthorized( operationType, comment, userSession ) )
          comment.setArchived( Boolean.valueOf( requestParams.getStringParameter( "value", "true" ) ) );
        break;
    }
