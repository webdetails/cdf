...
public class CdfHeadersProvider {
  ...
  private static CdfHeadersProvider defaultInstance = null;

  // TODO: temporary, move to another object
  public static void init() {
    defaultInstance = new CdfHeadersProvider();
  }
  public static CdfHeadersProvider getDefaultInstance() {
   ...
   * @param dashboardType blueprint|mobile
   * @param isDebugMode will concatenate/minify files if false
   ...
   * @param dashboardType blueprint|mobile
   * @param isDebugMode will concatenate/minify files if false
   * @param absRoot if you really need to add protocol+domain for some reason
   * @return html script/style includes
   */
  ...
  private List<StaticDependenciesPackage> createDependenciesPackages ( String pkgBaseName, PathSet pathSet ) {
    ...
    if ( !pathSet.ie8Styles.isEmpty() ) {
      ...
      dependencies.add(
          new IE8StyleDependencies( name, getContentAccess(), getUrlProvider(), origin, pathSet.ie8Styles ) );
    }
    return dependencies;
  }
  ...
  private StaticDependenciesPackage createDependencyPackage(
      String name, PackageType pkgType, PathOrigin origin, List<String> fileNames ) {
    return new StaticDependenciesPackage( name, pkgType, getContentAccess(), getUrlProvider(), origin,
