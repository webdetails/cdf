...
public class DialWidgetDefinition extends WidgetDefinition implements ChartDefinition {
   ...
   * @param session
   */
  public DialWidgetDefinition(IPentahoResultSet data, boolean byRow, Node chartAttributes, int width, int height,
    ...
    createDial(this, chartAttributes, width, height, session);
  }
  ...
  public DialWidgetDefinition(Document document, double value, int width, int height, IPentahoSession session) {
    ...
    createDial(this, attributes, width, height, session);
  }
   ...
   */
  public static void createDial(DialWidgetDefinition widgetDefinition, Node dialNode, int width, int height,
      IPentahoSession session) {
    ...
    if (paint == null) {
      ...
      getLogger().warn(Messages.getString("CHART.WARN_DEPRECATED_PROPERTY", "background-color","chart-background"));//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$    
      getLogger().warn(Messages.getString("CHART.WARN_PROPERTY_WILL_NOT_VALIDATE", "background-color"));//$NON-NLS-1$ //$NON-NLS-2$     
    }
    ...
    if (paint == null) {
      ...
      getLogger().warn(Messages.getString("CHART.WARN_DEPRECATED_PROPERTY", "plot-background-color","plot-background"));//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$    
      getLogger().warn(Messages.getString("CHART.WARN_PROPERTY_WILL_NOT_VALIDATE", "plot-background-color"));//$NON-NLS-1$ //$NON-NLS-2$     
    }
    ...
    if ((intervals == null) ||(intervals.isEmpty())){
      ...
      getLogger().warn(Messages.getString("CHART.WARN_DEPRECATED_CHILD", "interval","intervals"));//$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$    
      getLogger().warn(Messages.getString("CHART.WARN_PROPERTY_WILL_NOT_VALIDATE", "interval"));//$NON-NLS-1$ //$NON-NLS-2$     
    }
    ...
    List subtitles = dialNode.selectNodes(SUBTITLE_NODE_NAME); 
    
    if ((subtitles == null) ||(subtitles.isEmpty())){
      Node subTitlesNode = dialNode.selectSingleNode(SUBTITLES_NODE_NAME); 
  ...
  private void setDataByRow(IPentahoResultSet data) {
    ...
    double newValue = ((Number) rowData[0]).doubleValue();

    // Do we have a value, minimum and maximum? 
    if (rowData.length >= 3) {

      double newMinimum = ((Number) rowData[1]).doubleValue();
      double newMaximum = ((Number) rowData[2]).doubleValue();
      this.setMinimum(newMinimum);
