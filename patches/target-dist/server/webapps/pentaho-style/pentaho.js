...
function flyTableSetup( title, desc, iconSrc, target, solution, path, filename, furl, properties ) {
	...
	var actions = getOptions(solution, path, filename, target, filename=='', furl, properties);
	if( isAdmin == 'true' ) {
...
function showFly( obj, event, width, height, title, desc, iconSrc, target, solution, path, filename, furl, properties ) {
	hideFly2();
	var pos = getFlyXY( obj, event, 502, 203); 
	var top=pos[1]; 
	var left=pos[0]; 
	flyTableSetup( title, desc, iconSrc, target, solution, path, filename, furl, properties); 
	flyStatus = POPPING;
