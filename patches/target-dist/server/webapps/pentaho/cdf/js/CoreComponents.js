		...
		if(cd.events && cd.events.show == true){
			...
			timeplot.loadText(timePlotEventSourceUrl,",", timePlotEventSource, null,null,function(range){
				timeplot.loadJSON(eventUrl,eventSource2,function(data){
					data.events = this.FilterEvents(data.events, range);
...
var TextInputComponent = BaseComponent.extend({
	update : function() {
		// selectHTML = "<input onChange='Dashboards.processChange(\"" +
		// object.name + "\")' onKeyUp='if
		// (event.keyCode==13){Dashboards.processChange(\"" + object.name +
		// "\")}'";
		selectHTML = "<input";
		...
		$(function(){ 
			$("#" + myself.htmlObject + " input").daterangepicker({
					...
					onDateSelect: function(rangeA, rangeB) { 
						Dashboards.fireDateRangeInputChange( myself.name, rangeA, rangeB);
					}
				}); 
		});
	},
	fireDateRangeInputChange : function(rangeA, rangeB){
		// WPG: can we just use the parameter directly?
		var parameters = eval(this.name + ".parameter");
...
var TableComponent = BaseComponent.extend({
	...
	processTableComponentResponse : function(json)
	{
		...
		var dtData0 = this.getDataTableOptions(cd);
		...
		dtData.fnFinalCallback = function( aData, iRowCount ){
			$("#" + this.htmlObject + " td.sparkline").each(function(i){
			...
			if(cd.urlTemplate != undefined){
				...
				td.bind("click", function(e){
						...
						/*
						 * alert (cd.parameterName + " - " + $(this).text() + " - " +
						 * f);
						 */
						eval(f);
					});
...
var QueryComponent = BaseComponent.extend({
	update : function() {
		...
		var myself = this;
		$.getJSON("ViewAction?solution=cdf&path=components&action=jtable.xaction", cd, 
			function(json){
				myself.result = json;
			});
	}
