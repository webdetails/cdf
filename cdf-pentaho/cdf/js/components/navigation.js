...
var NavigatorBaseComponent = BaseComponent.extend({},{
  ...
  browseContent : function(files, currentPath) {
    ...
    Dashboards.error("Fatal: path " + NavigatorBaseComponent.path +" not found in navigation object");
    return;
  },
...
var ContentListComponent = NavigatorBaseComponent.extend({
  update : function() {
    var myself = this;
    var path = myself.mode != 4 ? NavigatorBaseComponent.path : NavigatorBaseComponent.getParentPath();
  ...
  processContentListResponse : function(json) {
    ...
    var container = $("<ul></ul>").attr("id", "contentList-" + myself.name).appendTo("#" + myself.htmlObject);

    // We need to append the parent dir
    if(myself.mode != 1 && myself.mode != 4 && NavigatorBaseComponent.path != "") {
      var parentDir =  {
        name: "Up",
        title: "Up",
        type: "FOLDER",
    ...
    $('#contentList-' + myself.name + ' a').tooltip({
      showURL: false
    });
...
var PageTitleComponent = NavigatorBaseComponent.extend({
  update : function() {
    ...
    if(NavigatorBaseComponent.navigatorResponse == -1 ) {
      ...
      myself.processPageTitleResponse(NavigatorBaseComponent.navigatorResponse);
    }
  },
